...
import { updateScreenShoot } from "../api/updatescreent";

function UpdateScreen({id}: {id: number}) {
  const [imageUrl, setImageUrl] = useState("");
  const [imageError, setImageError] = useState(false);

  const context = useContext(DashboardContext);
  const {state, updateScreenShoot: upScreenshoot} = context as ContextProps;
const { isPending,mutate } = useMutation({
    mutationFn: async (newScreen: Partial<SreenShoot>) =>
      await updateScreenShoot(newScreen, id),
    onSuccess: (data: any) => {
      upScreenshoot(data.data);
      setImageUrl("")
      setImageError(false)
    },
  });
  ...
  useEffect(() => {
    const scrennshoot= state.screeShoot.find(value => value.id === id)
    if(scrennshoot) {
      setImageUrl(scrennshoot?.screenshots)
    }
  }, [id, state.screeShoot])
